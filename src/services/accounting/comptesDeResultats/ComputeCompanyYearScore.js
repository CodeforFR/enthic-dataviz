...
function computeTransparencyScore(yearTreeData) {
  ...
  if (
    checkValueExist(yearTreeData.children.ParticipationSalariesAuxResultats)
  ) {
  ...
  if (
    checkValueExist(
      yearTreeData.children.ResultatAvantImpot.children.ResultatFinancier
    )
  ) {
    transparencyScore += 2.5;
  }
  ...
  if (
    checkValueExist(
      yearTreeData.children.ResultatAvantImpot.children.ResultatExploitation
    )
  ) {
    transparencyScore += 5.0;
  }
...
function computeSharingScore(yearTreeData) {
  ...
  var resultatCapitaux =
    ...
    yearTreeData.children.ResultatAvantImpot.children.ResultatFinancier.data
      .value;
  var benefice = yearTreeData.data.value;
  var resultatExploitation =
    yearTreeData.children.ResultatAvantImpot.children.ResultatExploitation.data
      .value;
  if (sharedPart > 0) {
...
function computeRealEconomyScore(yearTreeData) {
  var charges =
    yearTreeData.children.ResultatAvantImpot.children.ResultatExploitation
      .children.ChargesExploitation.children;
  return (
    (charges.SalairesEtTraitements.data.value +
      ...
      yearTreeData.children.ResultatAvantImpot.children.ResultatExploitation
        .children.ProduitsExploitation.children.SubventionsExploitation.data
        .value) /
    yearTreeData.children.ResultatAvantImpot.children.ResultatExploitation
      .children.ChargesExploitation.data.value
  );
}
