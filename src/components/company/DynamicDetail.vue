            ...
            <div class="tile is-ancestor is-vertical">
              ...
              <div class="tile is-parent">
                ...
                <p class="tile" style="color: #419">
                  Une valeur en bleu est une valeur non fournie mais qui peut
                  être retrouvée avec les autres valeurs
                </p>
          ...
          <div class="column">
            <div class="box">
              ...
              <div class="description">
                ...
                <div
                  id="block-main-tags"
                  v-if="isPositionFilled('block_main_tags')"
                >
                  <span
                  ...
                  <span
                    class="tag"
                    v-for="(tag, i) in matchProjectWithConfig(
                      'block_main_tags'
                    )"
                    :key="tag + i"
                  >
                ...
                <div
                  id="block-partners"
                  v-if="isPositionFilled('block_partners')"
                >
          ...
          <div class="column">
            <div class="box has-background-grey-lighter">
              ...
              <div
                class="added"
                id="block-src"
                v-if="isPositionFilled('block_src')"
              >
                <div class="columns">
                  <div class="column is-12">
                    <div>
    ...
    tidyRepresentants() {
      ...
      for (const representant of this.company.RncsImr.representants) {
        ...
        if (
          ...
          if (
            representantsStreamlined["P.Physique"].has(
              representant.id_representant
            )
          ) {
            representantsStreamlined["P.Physique"]
              .get(representant.id_representant)
              .push(representant);
          } else {
            representantsStreamlined[
              "P.Physique"
            ].set(representant.id_representant, [representant]);
          }
        }
        // Personne morale
        else if (representant.type_representant == "P. Morale") {
          if (
            representantsStreamlined["P. Morale"].has(representant.denomination)
          ) {
            representantsStreamlined["P. Morale"]
              .get(representant.denomination)
              .push(representant);
          } else {
            representantsStreamlined["P. Morale"].set(
              representant.denomination,
              [representant]
            );
          }
        } else {
      ...
      console.log(
        " - - DynamicDetail / computed / tidyRepresentants :",
        representantsStreamlined
      );
      ...
      for (var physique of representantsStreamlined["P.Physique"]) {
        ...
        var nodeLabel =
          representantPhysique.nom_patronyme +
          " " +
          representantPhysique.prenoms;
        ...
        for (var rep of physique[1]) {
          ...
          result.links.push({
            ...
            name:
              rep.qualite +
              "\n au " +
              representantPhysique.date_derniere_modification,
            _color: "red",
          });
        }
      }
      ...
      for (var morale of representantsStreamlined["P. Morale"]) {
        var nodeId2 = morale[0];
        var nodeColor2 = "f0f";
        var nodeLabel2 = morale[0] + "\n représenté par ";
        if (morale[1][0].siren_pm != null) {
          nodeColor2 = "f0f";
          nodeId2 = morale[1][0].siren_pm;
        }
        var qualiteMap = new Map();
        for (var rep2 of morale[1]) {
          if (qualiteMap.has(rep2.qualite)) {
            qualiteMap.set(rep2.qualite, qualiteMap.get(rep.qualite) + 1);
          } else {
            qualiteMap.set(rep2.qualite, 1);
          }
        }
        for (var qualite of qualiteMap) {
          nodeLabel2 += qualite[1] + " " + qualite[0] + ";";
        }
        var newNode2 = {
          id: nodeId2.toString(),
          name: nodeLabel2,
