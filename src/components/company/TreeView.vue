...
export default {
  ...
  computed: {
    ...
    getColor: function () {
      if (
        this.item.data.status == "official" ||
        this.item.data.status == "checked"
      ) {
        return 494;
    ...
    getText: function () {
      ...
      text += isNaN(this.item.data.value)
        ? "non fourni"
        : formatter.format(this.item.data.value);
        ...
        text +=
          ...
          formatter.format(
            this.item.data.value
              ? this.item.data.value - this.item.data.computedValue
              : this.item.data.computedValue
          );
      }
